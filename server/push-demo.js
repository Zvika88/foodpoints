...
// Add our custom routes
    ...
    app.post('/notify/all', function (req, res, next) {
      ...
      PushModel.notifyByQuery({ appId: 'loopback-component-push-app'}, note, function (err) {
        if (err) {
          console.error('Cannot notify: %s', err.stack);
          next(err);
          return;
        }
...
// You should tweak config options in ./config.js
    ...
    var config = options.config || ConfigLoader.loadAppConfig(appRootDir, env);
